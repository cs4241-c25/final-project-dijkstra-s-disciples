import React from 'react';

const API_BASE_URL = 'https://cuesportspro.glitch.me';


const SignIn = () => {
    return (
        <section className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white">
            <div className="bg-gray-800 p-8 rounded-lg shadow-lg max-w-md text-center">
                <h1 className="text-2xl font-semibold mb-6">Sign in</h1>

                <div className="flex justify-center space-x-4 mt-8">
                    <a href="https://cuesportspro.glitch.me/login/federated/google"
                       className="bg-white text-gray-800 px-6 py-3 rounded-lg shadow-md hover:shadow-lg transition flex items-center justify-center space-x-2">
                        <svg width="20" height="20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z" fill="#4285F4"/>
                        </svg>
                        <span className="ml-2">Sign in with Google</span>
                    </a>
                </div>
            </div>

        </section>
    );
};

export default SignIn;
